    ...
    <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
        ...
        <form class="space-y-4 md:space-y-6" method="POST" action="{{route('login')}}" novalidate>
            ...
            <div>
                ...
                <input type="password" name="password" id="password" placeholder="••••••••" class="bg-gray-50 border text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:text-black  dark:focus:ring-blue-500 dark:focus:border-blue-500 @error('email') border-red-500 @enderror" value="{{old('password')}}"   placeholder="name@company.com"  required="">
                @error('password')
                    <p class="bg-red-500 text-white my-2 rounded-lg text-sm p-2 text-center">{{$message}}</p>
                @enderror
            </div>
        ...
        <div class="mt-4 text-center">
            <a href="{{ route('welcome') }}" class="text-gray-600 dark:text-black underline">Regresar</a>
        </div>
    </div>
</div>
@endsection